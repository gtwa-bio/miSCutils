...
results_DESeq <- function(DESeq_obj, seurat_obj, Contrast) {
    ...
    cells.1 <- seurat_obj@meta.data %>%
        filter(Contrast[1] == Contrast[2]) %>%
        row.names()
    cells.2 <- setdiff(x = row.names(seurat_obj@meta.data), y = cells.1)
    ...
    res <- res %>%
        select(
            CellType, GeneName, baseMean, log2FoldChange, lfcSE, stat, pvalue,
            padj, Pct_CellType_Expressing, Pct_Other_Expressing
        )
